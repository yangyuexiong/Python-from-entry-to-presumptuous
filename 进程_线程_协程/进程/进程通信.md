...
```text
与多线程通信的区别:
    (1)不能使用queue.Queue,应该使用 multiprocessing.Queue
    (2)无法共享全局变量,多进程中数据是互相隔离的
    (3)multiprocessing.Queue无法在进程池中通信,需要使用 multiprocessing.Manager().Queue()
    (4)Pipe 实现进程池通信,但只能适用于2个进程
    (5)Pipe 性能高于Queue
```

- [process_communication_compared.py](./code/process_communication_compared.py)
